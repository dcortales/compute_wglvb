...
ax0 = axs[0]
ax0.set_title(r'(a) Absolute relative error between time-mean $w_{g}$ and $w_{tot}$ at $\sigma$ '+str(round(isopl[0,k],2))+ ' kg m$^{-3}$',fontsize = 28)
...
ax1 = axs[1]
ax1.set_title(r'(b) Correlation coefficient between annual $w_{g}$ and $w_{tot}$ at $\sigma$ '+str(round(isopl[0,k],2))+ ' kg m$^{-3}$',fontsize = 28)
...
ee_int = (wtot_int-wlvb_int)/abs(wtot_int)

ax0 = axs[2]
ax0.set_title(r'(c) Relative difference in time-mean vertical gradient ($\Delta_z w_{tot}$ - $\Delta_z w_{g}$)/|$\Delta_z w_{tot}$|',fontsize = 28)
ax0.set_xlabel(r'$Longitude(^o)$')
ax0.set_ylabel(r'$Latitude(^o)$')
